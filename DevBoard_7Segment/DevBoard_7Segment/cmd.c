...
void		cmdInit				( cmd_t *c )					
{
	...
	c->Data		= NULL; // Zeiger auf Nutzdaten
}
...
uint8_t		cmdParse			( uint8_t *rx , cmd_t *c )		
{
	...
	c->DataLength 	= rx[ FrameStart + CMD_HEADER_LENGHT		] -__CMD_HEADER_ENTRYS__;
	c->DataType		= rx[ FrameStart + CMD_HEADER_DATA_TYP		];
	c->MessageID 	= rx[ FrameStart + CMD_HEADER_ID			];
	c->Exitcode		= rx[ FrameStart + CMD_HEADER_Exitcode		];	
	MasterFrameCRC	= rx[ FrameStart + CMD_HEADER_CRC			];
	
	if ( c->DataLength )
	{
		c->Data = rx + ( FrameStart + __CMD_HEADER_ENTRYS__ );
	}
	else
	{
		c->Data = NULL; // Keine Nutzdaten
	}
	
	uint8_t crc = 0;
	SlaveFrameCRC = 0;
	
	rx[ FrameStart + CMD_HEADER_CRC ] = 0;
	
	for ( uint8_t x = 0 ; x < __CMD_HEADER_ENTRYS__ ; x++ )
	{
		crc = cmdCrc8CCITTUpdate( crc , &rx[ FrameStart + ( CMD_HEADER_LENGHT ) + x ] );
	}
	
	for ( uint8_t x = 0 ; x < c->DataLength ; x++ )
	{
		crc = cmdCrc8CCITTUpdate( crc , &rx[  ( FrameStart + __CMD_HEADER_ENTRYS__ ) + x ] );
	}
	SlaveFrameCRC = crc;
